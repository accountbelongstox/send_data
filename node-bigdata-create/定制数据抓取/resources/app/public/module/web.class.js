...
class C{
    ...
    changeSupportWeb(element,callback){
        ...
        that.load.module.function.createReadDataHtml();
        that.load.eles.selectSupportWeb.hide();
        //当前要抓取的WEB URL
    ...
    changeReadDataURL(){
        let
            that = this
        ;
        that.load.module.function.getCurrentReadObject();
    }
	...
	init(){
		...
		o = {
		    init:function(){
                ...
                //生成支支持网站的选项
                that.load.module.function.createSupportWebHtml();
                //生成网站的数据源选项
                that.load.eles.selectReadData.html(that.load.module.function.createReadDataHtml());
                if(supportHTML.includes("载入中")){
                    //首次给默认的支持 网站
                    that.load.eles.supportWeb.html(that.load.module.function.firstSupportWebURL());
                }
            },
