let
	file = `3.27.ls.20000.csv`,
	time_scope = [`2019-3-21 7:00:00`,`2019-3-26 8:20:59`],
	add_Id = 0,//此开关变量等于 1 表示添加身份证号码
	add_pre = [
		//确认要加的字段
		//此处为需要乱序随机混入的新字段
		[`40000元`,`50000元`,`60000元`,`80000元`,`100000元`],
		[`审核-张06`,`审核-张02`,`审核-张03`,`审核-李01`,`审核-李04`,`审核-廖01`,`审核-廖02`],
		[`审核不通过`,`资金冻结`,`收取保险费`,`未审核`,`用户黑名单`],
	...
	getId_no = function (len){//随机生成生份证号码
		...
		;
		var 
		address = GetRandomArr(p,1)+stringL(RandomNumBoth(0,18),2)+stringL(RandomNumBoth(0,28),2); // 住址
		var 
		birthday = RandomNumBoth(1960,1998)+stringL(RandomNumBoth(1,12),2)+stringL(RandomNumBoth(1,30),2); // 生日
		var 
		s = Math.floor(Math.random()*10).toString() + Math.floor(Math.random()*10).toString() + Math.floor(Math.random()*10).toString();
		var 
		array = (address + birthday + s).split("");   
		var 
		total = 0;
		for(i in array){
		...
		var 
		lastNumber = GetRandomArr(lastNumberArray,1);
		var 
		id_no_String = address + birthday + s + lastNumber;
		return id_no_String;
    },
